...
async def ask_openai(*, event, say, app):
    ...
    reply_ts = reply["ts"]
    history = await fetch_conversations_history(app.client, channel) # TODO: retrieval is the name of the game. Enhance this with proper search.
    logger.debug(f"history: {history}")
    openai_response = await ask(q=message)
